...
/**
 * Represents a "name" - "value" pair, used to represent Java annotation parameters.
 */
data class NameValue(val name: String, val value: Any)

/**
 * Create a "reason" string parameter as a list.
 */
fun reasonParam(reason: String): List<NameValue> {
    return listOf(NameValue("reason", reason))
}
...
abstract class UnifiedVisitor {
    abstract fun visitAnnotation(descriptor: String, visible: Boolean, params: List<NameValue>)
        ...
        fun on(target: ClassVisitor): UnifiedVisitor {
            return object : UnifiedVisitor() {
                override fun visitAnnotation(
                    ...
                    target.visitAnnotation(descriptor, visible).addParams(params)
                }
            }
        }
