...
class ShutdownGlobalActionViewModelTest : SysuiTestCase() {

    private val kosmos = testKosmosNew()
    private val Kosmos.underTest by
        Kosmos.Fixture {
            shutdownGlobalActionViewModelFactory.create().apply { activateIn(testScope) }
        }
    ...
    fun state_isVisible() =
        kosmos.runTest {
            ...
            assertThat(underTest.state.key).isEqualTo(GlobalActionType.POWER)
            assertThat(underTest.state.textResId).isEqualTo(R.string.global_action_power_off)
            assertThat(underTest.state.icon).isInstanceOf(Icon.Resource::class.java)
            assertThat(underTest.state.icon.resId)
                .isEqualTo(com.android.systemui.res.R.drawable.ic_global_actions_power_off)
        }

    @Test
    fun onClick_triggersShutdown() =
        kosmos.runTest {
            underTest.state.onClick()

            verify(kosmos.globalActionsManager).shutdown()
        }
    ...
    fun onClick_logsUiEvent() =
        kosmos.runTest {
            ...
            assertThat(uiEventLoggerFake.eventId(0))
                .isEqualTo(GlobalActionsEvent.GA_SHUTDOWN_PRESS.id)
        }
    ...
    fun onLongClick_logsUiEvent() =
        kosmos.runTest {
            ...
            assertThat(uiEventLoggerFake.numLogs()).isEqualTo(1)
            assertThat(uiEventLoggerFake.eventId(0))
                .isEqualTo(GlobalActionsEvent.GA_SHUTDOWN_LONG_PRESS.id)
        }
}
