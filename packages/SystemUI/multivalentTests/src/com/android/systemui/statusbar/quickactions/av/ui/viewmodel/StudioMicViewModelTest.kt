...
class StudioMicViewModelTest : SysuiTestCase() {
    ...
    fun titleAndIcon() =
        kosmos.runTest {
            assertThat(underTest.state.subText)
                .isEqualTo(com.android.systemui.res.R.string.av_studio_mic)
            assertThat(underTest.state.image)
                .isEqualTo(com.android.systemui.res.R.drawable.gs_mic_detect_auto)
        }

    @Test
    fun initialState_studioMicDisabled() =
        kosmos.runTest { assertThat(underTest.state.isEnabled).isFalse() }

    @Test
    fun studioMicEnabled_stateEnabled() =
        kosmos.runTest {
            kosmos.fakeSettings.putBool(DESKTOP_EFFECTS_STUDIO_MIC_KEY, true)
            assertThat(underTest.state.isEnabled).isTrue()
        }

    @Test
    fun onClick_togglesStudioMic() =
        kosmos.runTest {
            ...
            // Click to enable
            underTest.onClick()
            assertThat(kosmos.fakeSettings.getBool(DESKTOP_EFFECTS_STUDIO_MIC_KEY)).isTrue()
            assertThat(underTest.state.isEnabled).isTrue()

            // Click to disable
            underTest.onClick()
            assertThat(kosmos.fakeSettings.getBool(DESKTOP_EFFECTS_STUDIO_MIC_KEY)).isFalse()
            assertThat(underTest.state.isEnabled).isFalse()
        }
}
