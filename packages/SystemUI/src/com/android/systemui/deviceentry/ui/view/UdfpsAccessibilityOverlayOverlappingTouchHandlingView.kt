...
/**
 * For a TouchHandlingView that overlaps with the UDFPS accessibility overlay. This view type
 * intercepts touch events within the bounds of the accessibility overlay view, to prevent
 * overlapping / interfering talkback messages.
 */
class UdfpsAccessibilityOverlayOverlappingTouchHandlingView(
    context: Context,
    attrs: AttributeSet?,
    longPressDuration: () -> Long,
    allowedTouchSlop: Int = ViewConfiguration.getTouchSlop(),
    logger: TouchHandlingViewLogger? = null,
) : TouchHandlingView(context, attrs, longPressDuration, allowedTouchSlop, logger) {
