...
/**
 * Handles two ripple effects: dwell ripple and unlocked ripple Dwell Ripple:
 ...
 * - fadeDwellRipple: fades the dwell ripple away to alpha 0 Unlocked ripple:
...
class AuthRippleView(context: Context?, attrs: AttributeSet?) : View(context, attrs) {
    ...
    fun retractDwellRipple() {
        ...
        if (dwellPulseOutAnimator?.isRunning == true) {
            val retractDwellRippleAnimator =
                ValueAnimator.ofFloat(dwellShader.progress, 0f).apply {
    ...
    fun fadeDwellRipple() {
        ...
        if (dwellPulseOutAnimator?.isRunning == true || retractDwellAnimator?.isRunning == true) {
            fadeDwellAnimator =
                ValueAnimator.ofInt(Color.alpha(dwellShader.color), 0).apply {
                    interpolator = Interpolators.LINEAR
    ...
    fun startDwellRipple(isDozing: Boolean) {
        ...
        val dwellPulseOutRippleAnimator =
        ...
        dwellPulseOutAnimator =
            AnimatorSet().apply {
                playSequentially(dwellPulseOutRippleAnimator, expandDwellRippleAnimator)
                addListener(
                    object : AnimatorListenerAdapter() {
                        override fun onAnimationStart(animation: Animator) {
                            retractDwellAnimator?.cancel()
    ...
    fun setLockScreenColor(color: Int) {
        ...
        rippleShader.color = ColorUtils.setAlphaComponent(lockScreenColorVal, 62)
    }
    ...
    fun resetDwellAlpha() {
        dwellShader.color = ColorUtils.setAlphaComponent(dwellShader.color, 255)
    }
    ...
    override fun onDraw(canvas: Canvas) {
        ...
        if (drawDwell) {
            val maskRadius =
                (1 -
                    (1 - dwellShader.progress) *
                        (1 - dwellShader.progress) *
                        (1 - dwellShader.progress)) * dwellRadius * 2f
            canvas?.drawCircle(
                dwellOrigin.x.toFloat(),
                dwellOrigin.y.toFloat(),
                maskRadius,
                dwellPaint,
            )
        }
        ...
        if (drawRipple) {
            canvas?.drawCircle(
                origin.x.toFloat(),
                origin.y.toFloat(),
                rippleShader.rippleSize.currentWidth,
                ripplePaint,
            )
        }
    }
