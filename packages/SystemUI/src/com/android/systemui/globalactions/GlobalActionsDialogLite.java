...
public class GlobalActionsDialogLite implements DialogInterface.OnDismissListener,
    ...
    void createActionItems() {
        ...
        List<GlobalActionType> actionTypes = mRebootMenu
                ? getRebootMenuActionTypes()
                : mInteractor.getPossibleGlobalActions();
    ...
    private List<GlobalActionType> getRebootMenuActionTypes() {
        List<GlobalActionType> actionTypes = new ArrayList<>();
        for (String actionKey : mRebootMenuActions) {
            GlobalActionType actionType = GlobalActionType.Companion.fromConfigKey(actionKey);
            if (actionType == null) {
                Log.e(TAG, "Unknown global action type in config_rebootActionsList: "
                        + actionKey);
                continue;
            }
