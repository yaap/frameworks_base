...
class PortraitRelightButtonViewModel
@AssistedInject
        ...
        desktopEffectInteractor.model
            .map {
                ButtonUiState(
                    ...
                    image = com.android.systemui.res.R.drawable.gs_lightbulb,
                )
            }
            .hydratedStateOf(initialValue = ButtonUiState())

    override suspend fun onClick() {
        desktopEffectInteractor.setPortraitRelight(newValue = !state.isEnabled)
    }

    /** A factory to be used to create view model instances. */
    @AssistedFactory
    interface Factory {
        fun create(): PortraitRelightButtonViewModel
    }
}
