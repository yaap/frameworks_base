...
class NotifCollectionLogger @Inject constructor(@NotificationLog private val buffer: LogBuffer) {
    fun logNotifPosted(entry: NotificationEntry) {
        buffer.log(TAG, INFO, { str1 = entry.logKey }, { "POSTED $str1" })
    }
    ...
    fun logNotifUpdated(entry: NotificationEntry) {
        buffer.log(TAG, INFO, { str1 = entry.logKey }, { "UPDATED $str1" })
    }
    ...
    fun logNotifReleased(entry: NotificationEntry) {
        buffer.log(TAG, INFO, { str1 = entry.logKey }, { "RELEASED $str1" })
    }
    ...
    fun logDismissAll(userId: Int) {
        buffer.log(TAG, INFO, { int1 = userId }, { "DISMISS ALL notifications for user $int1" })
    }
    ...
    fun logMissingNotifications(newlyMissingKeys: List<String>, totalMissing: Int) {
        buffer.log(
            TAG,
            WARNING,
            {
    ...
    fun logFoundNotifications(newlyFoundKeys: List<String>, totalMissing: Int) {
        buffer.log(
            TAG,
            INFO,
            {
    ...
    fun logLocallyDismissAlreadyDismissedNotif(entry: NotificationEntry, index: Int, count: Int) {
        buffer.log(
            TAG,
            INFO,
            {
