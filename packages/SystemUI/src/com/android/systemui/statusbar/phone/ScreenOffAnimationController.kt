...
@SysUISingleton
class ScreenOffAnimationController
@Inject
constructor(
    ...
    private val foldToAodAnimation: FoldAodAnimationController? =
        sysUiUnfoldComponent.orElse(null)?.getFoldAodAnimationController()
    ...
    /**
     * Called when opaqueness of the light reveal scrim has change When [isOpaque] is true then
     * scrim is visible and covers the screen
     */
    ...
    fun onAlwaysOnChanged(alwaysOn: Boolean) = animations.forEach { it.onAlwaysOnChanged(alwaysOn) }

    /**
     * If returns true it fully expands notification shade, it could be used to make the scrims
     * visible
     */
    fun shouldExpandNotifications(): Boolean = animations.any { it.isAnimationPlaying() }

    /** If returns true it will disable propagating touches to apps and keyguard */
    fun shouldIgnoreKeyguardTouches(): Boolean = animations.any { it.isAnimationPlaying() }

    /** If returns true wake up won't be blocked when dozing */
    fun allowWakeUpIfDozing(): Boolean = animations.all { !it.isAnimationPlaying() }

    /**
     * Do not allow showing keyguard immediately so it could be postponed e.g. to the point when the
     * animation ends
     */
    fun shouldDelayKeyguardShow(): Boolean = animations.any { it.shouldDelayKeyguardShow() }
     ...
     */
    fun isKeyguardShowDelayed(): Boolean = animations.any { it.isKeyguardShowDelayed() }

    /** Return true to ignore requests to hide keyguard */
    fun isKeyguardHideDelayed(): Boolean = animations.any { it.isKeyguardHideDelayed() }

    /** Return true to make the status bar expanded so we can animate [LightRevealScrim] */
    fun shouldShowLightRevealScrim(): Boolean = animations.any { it.shouldPlayAnimation() }
    ...
    /**
     * Return true to override the dozing state of the notifications to fully dozing, so the
     * notifications are not visible when playing the animation
     */
    fun overrideNotificationsFullyDozingOnKeyguard(): Boolean =
        animations.any { it.overrideNotificationsDozeAmount() }

    /**
     * Return true to clamp screen brightness to 'dimmed' value when devices times out and goes to
     * sleep
     */
    fun shouldClampDozeScreenBrightness(): Boolean = animations.any { it.shouldPlayAnimation() }

    /** Return true to show AOD icons even when status bar is in 'shade' state (unlocked) */
    fun shouldShowAodIconsWhenShade(): Boolean = animations.any { it.shouldShowAodIconsWhenShade() }
     ...
     */
    fun shouldAnimateDozingChange(): Boolean = animations.all { it.shouldAnimateDozingChange() }

    /**
     * Returns true when moving display state to power save mode should be delayed for a few
     * seconds. This might be useful to play animations in full quality, without reducing FPS.
     */
