...
class IssueRecordingServiceLegacy
@Inject
constructor(
    ...
    /**
     * It is necessary to bind to IssueRecordingServiceLegacy from the Record Issue Tile because
     * there are instances where this service is not created in the same user profile as the record
     * issue tile aka, headless system user mode. In those instances, the TraceurConnection will be
     * considered a leak in between notification actions unless the tile is bound to this service to
     * keep it alive.
     */
    ...
    override fun provideRecordingServiceStrings(): RecordingServiceStrings =
        IrsStringsLegacy(resources)
    ...
    companion object {
        private const val TAG = "IRServiceLegacy"
        ...
        fun getStopIntent(context: Context): Intent =
            Intent(context, IssueRecordingServiceLegacy::class.java)
...
private class IrsStringsLegacy(private val res: Resources) : RecordingServiceStrings(res) {
    override val title
