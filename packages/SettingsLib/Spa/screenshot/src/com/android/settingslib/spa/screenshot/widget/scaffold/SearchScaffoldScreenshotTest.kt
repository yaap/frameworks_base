...
class SearchScaffoldScreenshotTest(emulationSpec: DeviceEmulationSpec) {
  companion object {
    @Parameters(name = "{0}")
    @JvmStatic
  ...
  @Test
  fun test() {
    screenshotRule.screenshotTest("searchScaffold") {
      SearchScaffold(
          title = "All apps",
          actions = { MoreOptionsAction {} },
      ) { _, _ ->
        ZeroStatePreference(icon = Icons.Outlined.Apps, text = "Text", description = "Description")
      }
    }
  }
}
