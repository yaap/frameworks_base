...
/**
 * When the user is dragging an icon from Taskbar to add an app into split screen, we have a set of
 * rules by which we draw and move colored drop targets around the screen. The rules are provided
 * through this interface.
 *
 * Each possible screen layout should have an implementation of this interface. E.g. the following
 * would be three implementations of this interface:
 * - 50:50 two-app split
...
interface DropTargetAnimSupplier {
     ...
     *
     * All lists should have the SAME number of elements, even if no animations are to be run for a
     * given target while in a hover state. It's not that we don't trust you, but we _really_ don't
     * trust you, so this will throw an exception if lengths are different. Don't ruin it for
     * everyone else... or do. Idk, you're an adult.
     */
    fun getTargets(
