...
class BubbleDataRepository(
    ...
    fun addBubbles(@UserIdInt userId: Int, bubbles: List<Bubble>) {
        ...
        val entities = transform(bubbles).also { b -> volatileRepository.addBubbles(userId, b) }
    ...
    fun removeBubbles(@UserIdInt userId: Int, bubbles: List<Bubble>) {
        ...
        val entities = transform(bubbles).also { b -> volatileRepository.removeBubbles(userId, b) }
        if (entities.isNotEmpty()) persistToDisk()
    }
     ...
     *   on the main thread of the hosting process. The callback is only run if there are bubbles.
     */
    @SuppressLint("WrongConstant")
    fun loadBubbles(userId: Int, currentUsers: List<Int>, cb: (List<Bubble>) -> Unit) =
        coroutineScope.launch {
            /**
             * Load BubbleEntity from disk. e.g.
             *
             * ```
             * [
            ...
            // Before doing anything, validate that the entities we loaded are valid & have an
            // existing
            // user.
            ...
            /**
             * Extract userId/packageName from these entities. e.g.
             *
             * ```
             * [
             ...
             * mapping from the userId/packageName pair to a list of associated ShortcutInfo. e.g.
             *
             * ```
             * {
