 ...
 *
 * Build/Install/Run: atest WMShellUnitTests:FluidResizeTaskPositionerTest
 */
@SmallTest
...
class FluidResizeTaskPositionerTest : ShellTestCase() {
    ...
    fun testDragResize_notMove_skipsTransitionOnEnd() {
        ...
        verify(mockTransitions, never())
            .startTransition(
                eq(WindowManager.TRANSIT_CHANGE),
                argThat { wct ->
                ...
                argThat { wct ->
                    return@argThat wct.changes.any { (token, change) ->
                        token == taskBinder &&
                            ((change.windowSetMask and WindowConfiguration.WINDOW_CONFIG_BOUNDS) !=
                                0) &&
                            change.configuration.windowConfiguration.bounds.right ==
    ...
    fun testDragResize_toDisallowedBounds_freezesAtLimit() {
        ...
        val newBounds2 =
            Rect(newBounds.left, newBounds.top, newBounds.right + 10, DISALLOWED_RESIZE_AREA.top)
        taskPositioner.onDragPositioningMove(
            DISPLAY_ID,
    ...
    fun testIsResizingOrAnimatingResizeResetAfterAbortedTransition() {
        ...
        taskPositioner.onTransitionConsumed(
            mockTransitionBinder,
            true /* aborted */,
            mockTransaction,
        )
