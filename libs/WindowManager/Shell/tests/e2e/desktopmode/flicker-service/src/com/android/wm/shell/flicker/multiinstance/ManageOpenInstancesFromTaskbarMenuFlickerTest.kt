...
import com.android.wm.shell.flicker.utils.layerBecomesVisibleThenInvisible
...
/**
 * Test for managing open instances from the taskbar menu in Desktop Mode.
 *
 * Assert that the manage windows layer becomes visible then invisible.
 */
@RequiresDesktopDevice
@Postsubmit
@RunWith(Parameterized::class)
@Parameterized.UseParametersRunnerFactory(FlickerParametersRunnerFactory::class)
class ManageOpenInstancesFromTaskbarMenuFlickerTest(flicker: FlickerTest) :
    ...
    private val viewOpenInstancesMatcher: IComponentNameMatcher =
        ComponentNameMatcher.TASK_BAR_OVERLAY

    override val transition: FlickerBuilder.() -> Unit
        get() = {
            setup { scenario.setup() }
            transitions { scenario.triggerFromTaskbar() }
    ...
    @Test
    fun layerBecomesVisibleThenInvisible() =
        flicker.layerBecomesVisibleThenInvisible(viewOpenInstancesMatcher)
