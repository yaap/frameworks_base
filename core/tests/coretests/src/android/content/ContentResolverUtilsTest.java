...
public class ContentResolverUtilsTest {

    private IContentProvider mProvider;
    private ContentProviderClient mClient;

    private RandomAccessFile mImage;

    @Before
    public void setUp() throws Exception {
        final var resolver = InstrumentationRegistry.getInstrumentation().getTargetContext()
                .getContentResolver();
        mProvider = mock(IContentProvider.class);
        mClient = new ContentProviderClient(resolver, mProvider, false);

        mImage = new RandomAccessFile(File.createTempFile("temp", ".png"), "rw");
    }
    ...
    private void initImage(int width, int height) throws Exception {
        ...
        bitmap.compress(Bitmap.CompressFormat.PNG, 90, new FileOutputStream(mImage.getFD()));

        final AssetFileDescriptor afd = new AssetFileDescriptor(
                ParcelFileDescriptor.dup(mImage.getFD()), 0, mImage.length(), null);
    ...
    @Test
    public void testTranslateDeprecatedDataPath() {
