...
public class QwertyKeyListenerTest {
    ...
    private QwertyKeyListener mListener;
    private Editable mEditable;

    @Before
    public void setUp() {
        mListener = QwertyKeyListener.getInstance(false, Capitalize.NONE);
                        ...
                        () -> {
                            ...
                            KeyEvent event =
                                    new KeyEvent(
                                            ...
                                            1,
                                            0);
                            mListener.onKeyDown(null, mEditable, KeyEvent.KEYCODE_A, event);
                        ...
                        () -> {
                            ...
                            KeyEvent eventA =
                                    new KeyEvent(
                                            ...
                                            1,
                                            0);
                            mListener.onKeyDown(null, mEditable, KeyEvent.KEYCODE_A, eventA);
                            ...
                            KeyEvent eventB =
                                    new KeyEvent(
                                            ...
                                            1,
                                            0);
                            mListener.onKeyDown(null, mEditable, KeyEvent.KEYCODE_B, eventB);

                            assertEquals("ab", mEditable.toString());
    ...
    @EnableCompatChanges(ShowSecretsSetting.SPLIT_SHOW_PASSWORDS_TO_TOUCH_AND_PHYSICAL)
    @EnableFlags(Flags.FLAG_SPLIT_SHOW_PASSWORDS_TO_TOUCH_AND_PHYSICAL)
    public void testMixedKeys_correctlyAddsSpans() {
        InstrumentationRegistry.getInstrumentation()
                .runOnMainSync(
                        () -> {
                            RecordingSpanWatcher watcher = new RecordingSpanWatcher();
                            ...
                            KeyEvent physicalA =
                                    new KeyEvent(
                                            ...
                                            1,
                                            0);
                            mListener.onKeyDown(null, mEditable, KeyEvent.KEYCODE_A, physicalA);
                            ...
                            KeyEvent virtualB =
                                    new KeyEvent(
                                            ...
                                            -1,
                                            0);
                            mListener.onKeyDown(null, mEditable, KeyEvent.KEYCODE_B, virtualB);
                            ...
                            KeyEvent physicalC =
                                    new KeyEvent(
                                            ...
                                            1,
                                            0);
                            mListener.onKeyDown(null, mEditable, KeyEvent.KEYCODE_C, physicalC);

                            assertEquals("abc", mEditable.toString());

                            assertEquals(2, watcher.mAddedSpans.size());
                            assertEquals(2, watcher.mRemovedSpans.size());

                            // First span for 'a'
                            assertEquals(0, (int) watcher.mRemoveStarts.get(0));
                            assertEquals(1, (int) watcher.mRemoveEnds.get(0));

                            // Second span for 'c'
                            assertEquals(2, (int) watcher.mRemoveStarts.get(1));
                            assertEquals(3, (int) watcher.mRemoveEnds.get(1));

                            PhysicalInputSpan[] spans =
                                    mEditable.getSpans(0, 3, PhysicalInputSpan.class);
                            assertEquals(0, spans.length);
                        });
    }
    ...
    private static class RecordingSpanWatcher implements SpanWatcher {
        ...
        final List<Object> mRemovedSpans = new ArrayList<>();
        final List<Integer> mRemoveStarts = new ArrayList<>();
        final List<Integer> mRemoveEnds = new ArrayList<>();
        ...
        public void onSpanRemoved(Spannable text, Object what, int start, int end) {
            if (what instanceof PhysicalInputSpan) {
                mRemovedSpans.add(what);
                mRemoveStarts.add(start);
                mRemoveEnds.add(end);
            }
        }
