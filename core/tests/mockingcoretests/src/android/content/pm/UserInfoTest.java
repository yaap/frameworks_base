...
public final class UserInfoTest {
    ...
    // This test is automatically generated by AI.
    @Test
    public void testIsFull() {
        UserInfo fullUser = createTestUserInfo(FLAG_FULL);
        expect.withMessage("isFull() with only FLAG_FULL").that(fullUser.isFull()).isTrue();

        UserInfo fullAdminUser = createTestUserInfo(FLAG_FULL | FLAG_ADMIN);
        expect.withMessage("isFull() with FLAG_FULL and another flag")
                .that(fullAdminUser.isFull()).isTrue();

        UserInfo profileUser = createTestUserInfo(FLAG_PROFILE);
        expect.withMessage("isFull() with FLAG_PROFILE").that(profileUser.isFull()).isFalse();

        UserInfo noFlagUser = createTestUserInfo(/* flags= */ 0);
        expect.withMessage("isFull() with no flags").that(noFlagUser.isFull()).isFalse();
    }
