...
/**
 * Tests for non-SDK methods / internal behavior related to {@link TimeManager}. Also see {@link
 * android.app.time.cts.TimeManagerTest}
 */
...
public class TimeManagerTest {
     ...
     */
    @Rule public final AdoptShellPermissionsRule shellPermRule = new AdoptShellPermissionsRule();
    ...
    public void before() throws Exception {
        ...
        DeviceShellCommandExecutor shellCommandExecutor =
                new InstrumentationShellCommandExecutor(instrumentation.getUiAutomation());
        ...
        mDeviceConfigPreTestState =
                mDeviceConfigShellHelper.setSyncModeForTest(
    ...
    /**
     * Tests a server flag that can be used to change the "automatic time zone enabled" value for
     * devices where the user hasn't yet expressed a preference. The flag is only intended for use
     * during internal testing and therefore has not been included in CTS; it could be removed in
     * later releases. This test takes ~35s to run because the asynchronous operations involved
    ...
    public void testTimeZoneEnabledDefaultFlagBehavior() throws Exception {
        ...
        assumeTrue(
                capabilities.getConfigureAutoDetectionEnabledCapability() == CAPABILITY_POSSESSED);
        ...
        try {
            ...
            sleepForAsyncOperation();
            assertEquals(
                    initialAutoTzEnabled, mTimeZoneDetectorShellHelper.isAutoDetectionEnabled());
