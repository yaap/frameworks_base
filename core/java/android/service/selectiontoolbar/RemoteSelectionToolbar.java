...
public final class RemoteSelectionToolbar {
    ...
    private List<ToolbarMenuItem> layoutMainPanelItems(List<ToolbarMenuItem> menuItems,
            int toolbarWidth) {
        final ArrayList<ToolbarMenuItem> remainingMenuItems = new ArrayList<>();
        // add the overflow menu items to the end of the remainingMenuItems list.
        final ArrayList<ToolbarMenuItem> overflowMenuItems = new ArrayList();
        for (ToolbarMenuItem menuItem : menuItems) {
            if (menuItem.itemId != android.R.id.textAssist
        ...
        while (!remainingMenuItems.isEmpty()) {
            ToolbarMenuItem menuItem = remainingMenuItems.get(0);
            ...
            final boolean showIcon = isFirstItem && menuItem.itemId == R.id.textAssist;
    ...
    private static void updateMenuItemButton(View menuItemButton, ToolbarMenuItem menuItem,
        ...
        if (menuItem.icon == null || !showIcon) {
            ...
            buttonIcon.setImageDrawable(
                    menuItem.icon.loadDrawable(menuItemButton.getContext()));
            buttonText.setPaddingRelative(iconTextSpacing, 0, 0, 0);
        }
        final CharSequence contentDescription = menuItem.contentDescription;
