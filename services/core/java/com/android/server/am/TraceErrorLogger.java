...
public class TraceErrorLogger {
    ...
    HashMap<Integer, UUID> mAnrIdToErrorId = new HashMap<>();
    ...
    /**
     * Retrieve an existing ErrorId that corresponds to the provided anr id which
     * can be any integer generated by the systems that triggered the ANR.
     * @param anrId Id that will be used to identify the ErrorId.
     * @return UUID retrieved or generated for the provided {@code anrId}
     */
    public UUID getOrCreateErrorId(int anrId) {
        if (mAnrIdToErrorId.containsKey(anrId)) {
            return mAnrIdToErrorId.get(anrId);
        }
        UUID errorId = generateErrorId();
        mAnrIdToErrorId.put(anrId, errorId);
        return errorId;
    }
