...
public class LockSettingsService extends ILockSettings.Stub {
    ...
    private void unlockChildProfile(int profileHandle) {
        try (LockscreenCredential credential =
            ...
            doVerifyCredential(
                    credential, Primary, profileHandle, /* progressCallback= */ null, /* flags= */ 0);
     ...
     */
    @VisibleForTesting
    boolean isProfileWithTiedLock(int userId) {
    ...
    // TODO: Upstream updated their version of this method to use UsermanagerInternal, as opposed
    // to ours which uses UserManager. Review whether we should refactor downstream.
        ...
        synchronized (mSpManager) {
            final long protectorId =
                    isSpecialUserId(userId)
                            ...
                            : getCurrentLskfBasedProtectorId(userId, lockDomain);
    ...
    private VerifyCredentialResponse doVerifyTiedProfileChallenge(
        ...
        try (LockscreenCredential profilePassword = getDecryptedPasswordForUnifiedProfile(userId)) {
            // Unlock profile with unified lock
            return doVerifyCredential(profilePassword, Primary, userId, /* progressCallback= */ null, flags);
        } catch (UnrecoverableKeyException | InvalidKeyException | KeyStoreException
                | NoSuchAlgorithmException | NoSuchPaddingException
                | InvalidAlgorithmParameterException | IllegalBlockSizeException
                | BadPaddingException | CertificateException | IOException e) {
    ...
    private void onCredentialVerified(
            SyntheticPassword sp, @Nullable PasswordMetrics metrics, int userId, LockDomain lockDomain) {
        SecureLockDeviceServiceInternal secureLockDeviceServiceInternal =
                mInjector.getSecureLockDeviceServiceInternal();
        if (lockDomain == Primary && secureLockDeviceServiceInternal != null
                && secureLockDeviceServiceInternal.isSecureLockDeviceEnabled()) {
            onCredentialVerifiedInSecureLockDeviceMode(sp, metrics, userId);
        } else {
            onCredentialVerifiedInternal(sp, metrics, userId, lockDomain);
        }
    }
    ...
    private void onCredentialVerifiedInternal(SyntheticPassword sp,
            @Nullable PasswordMetrics metrics, int userId, LockDomain lockDomain) {
     ...
     */
    @Override
    public ParcelDuration getLockoutEndTime(@UserIdInt int userId, LockDomain lockDomain) {
        checkPasswordHavePermission();
        final long protectorId = getCurrentLskfBasedProtectorId(userId, lockDomain);
